...
_PATCH_BASE = "tools.session_context_gen.r3_validator"
...
class TestR3Validator(unittest.TestCase):
    def setUp(self):
        ...
        self._write_json(self.ledger_path, [{"chain_hash": "a" * 64}])
        self._write_json(self.rules_path, {"allowed_event_types": ["DATA_SYNC", "SESSION_START"]})

        self.sample_candidate = {"session_count": 52, "event_type": "DATA_SYNC"}
        self.sample_receipt = {
            "candidate_hash": r3._sha256_hex(self.sample_candidate),
            "verifier_summary": {"final_chain_hash": "a" * 64},
            "selected_last_known_good": {"session_count": 51},
        }
    ...
    def test_t4_lkg_session_count_greater_than_current_fail(self, mock_get):
        ...
        self.assertEqual(result["verdict"], "FAIL")
        self.assertTrue(result["quarantine_applied"])
        self.assertIn("LKG session_count exceeds actual /session/current session_count", result["failure_reasons"])
    ...
    @patch(f"{_PATCH_BASE}.shutil.move")
    @patch(f"{_PATCH_BASE}.requests.get")
