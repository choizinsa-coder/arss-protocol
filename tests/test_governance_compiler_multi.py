import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "tools"))
sys.path.insert(0, os.path.dirname(__file__))
from governance_compiler import (
    compile_governance, build_governance_state,
    build_bridge_projection, build_compiler_receipt,
)
from governance_compiler.governance_state_builder import VALID, INVALID, REVIEW
from _fixtures import make_approval, make_context, VALID_HASH, VALID_HASH_2, BAD_HASH


def test_multiple_approvals_all_valid():
    ctx = make_context(["EAG-1", "EAG-2", "EAG-3"])
    apps = [make_approval("EAG-1"), make_approval("EAG-2"), make_approval("EAG-3")]
    state = build_governance_state(ctx["eag_chain"], approvals=apps, session="S272")
    assert state["approval_count"] == 3
    assert state["chain_complete"] is True
    assert state["compiler_verdict"] == VALID

def test_multiple_with_one_invalid():
    ctx = make_context(["EAG-1", "EAG-2"])
    apps = [make_approval("EAG-1"), make_approval("EAG-2", event_hash=BAD_HASH)]
    state = build_governance_state(ctx["eag_chain"], approvals=apps, session="S272")
    assert state["compiler_verdict"] == INVALID
