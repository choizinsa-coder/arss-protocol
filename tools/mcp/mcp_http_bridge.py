...
def _build_write_tool_entries() -> list:
    return [
        ...
        {"name": "ask_jeni", "description": "[ASK] 제니(Gemini Governance Auditor)에게 질문 (caddy only)",
         "inputSchema": {"type": "object", "properties": {"actor_id": {"type": "string", "enum": [ASK_JENI_ALLOWED_ACTOR]}, "prompt": {"type": "string"}, "escalate": {"type": "boolean", "description": "상위 모델 호출 여부 (미표기=false)"}, "reason": {"type": "string", "description": "상위 모델 사용 사유 (escalate=true 시, 기록용)"}, "context": {"type": "string"}, "session": {"type": "string", "description": "세션 ID (예: S264). persistent memory 세션별 축적용."}}, "required": ["actor_id", "prompt"]}},
        {"name": "ask_domi", "description": "[ASK] 도미(OpenAI Design Architect)에게 설계 의뢰 (caddy only)",
         "inputSchema": {"type": "object", "properties": {"actor_id": {"type": "string", "enum": [ASK_DOMI_ALLOWED_ACTOR]}, "prompt": {"type": "string"}, "escalate": {"type": "boolean", "description": "상위 모델 호출 여부 (미표기=false)"}, "reason": {"type": "string", "description": "상위 모델 사용 사유 (escalate=true 시, 기록용)"}, "context": {"type": "string"}, "session": {"type": "string", "description": "세션 ID (예: S264). persistent memory 세션별 축적용."}}, "required": ["actor_id", "prompt"]}},
