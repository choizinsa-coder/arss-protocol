"""
append_incident.py
INCIDENT 항목을 session_journal.jsonl에 WORM append하는 CLI 스크립트
EAG: EAG-S229-JOURNAL-001
Version: 1.0.0

사용법:
  python append_incident.py --session 229 --incident-id INC-S229-001 \
      --type CADDY_OPTION_MENU --description "선택지 메뉴 제시"
...
- 중복 항목(details["id"] == incident_id) 존재 시 SKIP
"""
...
def incident_entry_exists(journal_path: Path, incident_id: str) -> bool:
    """해당 incident_id의 항목이 이미 존재하는지 확인."""
    if not journal_path.exists():
        return False
    with open(journal_path, encoding="utf-8") as f:
        for line in f:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Append INCIDENT entry to session_journal.jsonl"
    )
    parser.add_argument(
        "--session", type=int, required=True,
        help="Session number (e.g. 229)"
    )
    parser.add_argument(
        "--incident-id", type=str, required=True,
        help="Incident ID (e.g. INC-S229-001)"
    )
    parser.add_argument(
        "--type", type=str, required=True,
        help="Incident type (e.g. CADDY_OPTION_MENU)"
    )
    parser.add_argument(
        "--description", type=str, required=True,
        help="Incident description text"
    )
    parser.add_argument(
    ...
    # 중복 확인
    if incident_entry_exists(journal_path, incident_id):
        print(f"[SKIP] {incident_id} already exists in journal.")
        return 0
