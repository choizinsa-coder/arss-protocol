"""
append_eag.py
EAG 항목을 session_journal.jsonl에 WORM append하는 CLI 스크립트
EAG: EAG-S229-JOURNAL-001
Version: 1.0.0

사용법:
  python append_eag.py --session 229 --eag-id EAG-S229-JOURNAL-001 \
...
- 중복 항목(details["id"] == eag_id) 존재 시 SKIP
"""
...
def eag_entry_exists(journal_path: Path, eag_id: str) -> bool:
    """해당 eag_id의 항목이 이미 존재하는지 확인."""
    if not journal_path.exists():
        return False
    with open(journal_path, encoding="utf-8") as f:
        for line in f:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Append EAG entry to session_journal.jsonl"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eag-id", type=str, required=True,
        help="EAG ID (e.g. EAG-S229-JOURNAL-001)"
    )
    parser.add_argument(
        "--summary", type=str, required=True,
        help="EAG summary text"
    )
    parser.add_argument(
    ...
    # 중복 확인
    if eag_entry_exists(journal_path, eag_id):
        print(f"[SKIP] {eag_id} already exists in journal.")
        return 0
